...
enum Rank {TWO, THREE, FOUR, FIVE, SIX, SEVEN, EIGHT, NINE, TEN, JACK, QUEEN, KING, ACE, NOTHING1};
enum Suit {CLUB, DIAMOND, HEART, SPADE, NOTHING2};
...
public:
    ...
    {
        Card temp;
        temp.SetRank(NOTHING1);
        temp.SetSuit(NOTHING2);
        int j = -1;
        for(int i = 0; i < n; i++)
        {
...
public:
    Player()
    {
        for (int i = 0; i < 5; i++)
        {
            holds[i].SetRank(NOTHING1);
            holds[i].SetSuit(NOTHING2);
        }
    }
    ...
    void pick_a_card(DECKofCARDS &A)
    {
        holds[total_holds++] = A.pick_a_card();
    }

    void pick_a_card(DECKofCARDS &A, Suit s, Rank r)
    {
        Card temp = A.pick_a_card(s, r);
        if (temp.GetSuit() != NOTHING2 && temp.GetRank() != NOTHING1)
        {
